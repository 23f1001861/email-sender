...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!session?.user?.email) {
      console.warn("[SECURITY] Unauthorized access attempt to /api/emails/sent");
    ...
    try {
      response = await fetch(
        `${backendUrl}/api/emails/sent?userEmail=${encodeURIComponent(sanitizedRequestEmail)}`,
        { signal: controller.signal, cache: "no-store" }
      );
    } catch (err) {
      console.error("Backend fetch error (sent):", err);
    ...
    if (!response.ok) {
      ...
      console.error("Backend responded non-OK (sent):", error || response.statusText);
      return NextResponse.json(
        { error: error || "Failed to fetch sent emails" },
    ...
    console.error("Unhandled error in sent route:", error);
    return NextResponse.json(
      { error: "Unexpected error fetching sent emails" },
      { status: 500 }
    );
  }
}
